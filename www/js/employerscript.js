...
function EcaptureESuccess(mediaFiles) {
    var videoName = localStorage.getItem("jobID");
    var i, len;

    for (i = 0, len = mediaFiles.length; i < len; i++) {
        var mediaFile = mediaFiles[i];
        mediaFile.getFormatData(EgetFormatEDataSuccess, EgetFormatEDataError);
        ...
        //if (device.platform == 'Android') {
            window.requestFileSystem(LocalFileSystem.PERSISTENT, 0, function (fileSystem) {
                ...
                window.PKVideoThumbnail.createThumbnail(videoPath, thumbFile, function (s) {
                    EsendVideoEThumbnail(mediaFile.fullPath, thumbFile, videoName);
                }, function (e) {
                });
            });
...
function EsendVideoEThumbnail(videoPath, thumbSrc, vidThumbName) {
...
function EgetFormatEDataSuccess(mediaFileData) {
